...
```bash
conda create -n demo_project python=3.9
...
```
This command will create your personal repository under `/Repos/<username>/demo_project`.
...
- To trigger the release pipeline, get the current version from the `demo_project/__init__.py` file and tag the current code version:
```
